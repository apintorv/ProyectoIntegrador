import rclpy
from rclpy.node import Node
from sensor_msgs.msg import Image
from std_msgs.msg import String, Bool
from geometry_msgs.msg import Twist
import cv2
from cv_bridge import CvBridge
import numpy as np
from rclpy.qos import QoSProfile, QoSReliabilityPolicy, QoSHistoryPolicy, QoSDurabilityPolicy

class LineFollower(Node):
    def __init__(self):
        super().__init__('line_follower')
        
        self.twist = Twist()
        
        self.bridge = CvBridge()
        
        qos_profile = QoSProfile(
            reliability = QoSReliabilityPolicy.RMW_QOS_POLICY_RELIABILITY_BEST_EFFORT,
            history = QoSHistoryPolicy.RMW_QOS_POLICY_HISTORY_KEEP_LAST,
            depth = 10
        )
        
        self.subscription = self.create_subscription(Image,'/video_source/raw',self.image_callback,1)
        self.subscriber_class = self.create_subscription(String, '/Yolov8_Inference', self.class_callback, qos_profile)  #CHECAR 
        self.subscriber_color = self.create_subscription(String, 'color', self.callback_color, qos_profile)
        self.subscriber_flag = self.create_subscription(Bool, 'flag', self.callback_flag, qos_profile)
        self.KP = 0.5
        self.publisher_ = self.create_publisher(Twist, 'cmd_vel', 10)
        self.kernel = np.ones((5, 5), np.uint8)
        self.vmax = 0.05
        self.type = String()
        self.color = String()
        self.traffic_sign = Bool()
        
    def callback_flag(self, msg):
        self.traffic_sign = msg.data
        
    def class_callback(self, msg):
        self.type = msg.data
        
    def callback_color(self,msg):
        self.color = msg.data
        
    def image_callback(self, msg):
        # Convertir el mensaje de imagen a un formato OpenCV
        cv_image = self.bridge.imgmsg_to_cv2(msg, 'bgr8')
        cv_image = cv2.flip(cv_image, 0)
        cv_image = cv2.flip(cv_image, 1)

        height, width = cv_image.shape[:2]
        new_height = int(height * 0.9)
        new_width = int(width * 0.2)
        roi = cv_image[new_height:height, (width - new_width) // 2:width - (width - new_width) // 2]

        gray_roi = cv2.cvtColor(roi, cv2.COLOR_BGR2GRAY)
        roi_blur = cv2.GaussianBlur(gray_roi, (15, 15), 0)
        eroded = cv2.erode(roi_blur, self.kernel, iterations=1)
        dilated = cv2.dilate(eroded, self.kernel, iterations=1)
        _, binary_roi = cv2.threshold(dilated, 100, 125, cv2.THRESH_BINARY_INV)
        contours, _ = cv2.findContours(binary_roi, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        
        height, width = roi.shape[:2]
        cx_a = width // 2
        # cy_a = height
        # cv2.circle(roi, (cx_a, cy_a), 5, (0, 255, 0), -1)
        
        if self.type == 'Stop-pannels' or self.color == 'Red':
                self.twist.angular.z = 0.0
                self.twist.linear.x = 0.0
        else:        
            if self.traffic_sign:    
                # if self.type == 'Stop-pannels' or self.color == 'Red':
                #     self.twist.angular.z = 0.0
                #     self.twist.linear.x = 0.0
                if self.type == 'Toutdroit-pannels':
                    self.get_logger().info(f'Toutdroit-pannels')
                    self.twist.angular.z = 0.0
                    self.twist.linear.x = self.vmax
                    self.type = ' '
                elif self.type == 'Derecha-Pannels':
                    self.get_logger().info(f'Derecha-Pannels')
                    self.twist.angular.z = - self.vmax
                    self.twist.linear.x = self.vmax
                    self.type = ' '
                elif self.type == 'Izquierda-pannels':
                    self.get_logger().info(f'Izquierda-pannels')
                    self.twist.angular.z = self.vmax
                    self.twist.linear.x = self.vmax
                    self.type = ' '
                elif self.type == 'Travaux-pannel':
                    self.get_logger().info(f'Travaux-pannel')
                    self.twist.angular.z = self.twist.angular.z/2
                    self.twist.linear.x = self.twist.linear.x/2
                    self.type = ' '
                
                self.traffic_sign = False
                pass
                
            elif contours:
                line_contour = max(contours, key=cv2.contourArea)
                # cv2.drawContours(roi, [line_contour], -1, (0, 0, 255), thickness=2)

                # Calcular el centro del contorno
                M = cv2.moments(line_contour)
                if M['m00'] > 0:
                    cx = int(M['m10'] / M['m00'])
                    # cy = int(M['m01'] / M['m00'])
                    # cv2.circle(roi, (cx, cy), 5, (0, 255, 0), -1)
                    
                    error = cx - cx_a
                    # self.get_logger().info(f'Error: {error}')

                    if abs(error) > 3 :
                        w_r = np.clip(- self.KP * error , -self.vmax, self.vmax)
                        self.twist.angular.z = w_r
                        
                    if self.color == 'Yellow':
                        self.get_logger().info(f'Yellow')
                        self.twist.angular.z = self.twist.angular.z/2
                    elif self.color == 'Green':
                        self.twist.angular.z = 0.0
                        self.color = ' '
                        
                
                        
                    self.twist.linear.x = self.vmax  # Velocidad constante hacia adelante                        
                else: 
                    self.twist.linear.x = self.vmax ### 
            
            else:
                self.get_logger().info(f'No contours/signal detected')
                self.twist.angular.z = self.vmax
                self.twist.linear.x = self.vmax
            
        self.publisher_.publish(self.twist)       
        self.get_logger().info(f'w: {self.twist.angular.z}, x: {self.twist.linear.x}')
                
            
        

def main(args=None):
    rclpy.init(args=args)
    node = LineFollower()
    rclpy.spin(node)
    rclpy.shutdown()

if __name__ == '__main__':
    main()