...
class LineFollower(Node):
    def __init__(self):
        ...
        self.toutdroit_detected = False  # Add this line
    ...
    def image_callback(self, msg):
        ...
        if self.type == 'Stop-pannels' or self.type == 'Rouge-Sema':
            self.twist.angular.z = 0.0
            self.twist.linear.x = 0.0
        else:
            if self.traffic_sign or self.color_flag:
    ...
    def follow_line(self, contours, cx_a):
        ...
        # Filter contours to only consider vertical lines
        vertical_contours = [cnt for cnt in contours if self.is_vertical(cnt)]
        horizontal_contours = [cnt for cnt in contours if self.is_horizontal(cnt)]
        ...
        if vertical_contours:
            line_contour = max(vertical_contours, key=cv2.contourArea)
            M = cv2.moments(line_contour)
            if M['m00'] > 0:
                cx = int(M['m10'] / M['m00'])
                self.error = cx - cx_a
                ...
                self.twist.angular.z = np.clip(-control, -self.vmax, self.vmax)
    ...
    def is_vertical(self, contour):
        x, y, w, h = cv2.boundingRect(contour)
        return h > w  # Consider vertical if height is greater than width
    
    def is_horizontal(self, contour):
        x, y, w, h = cv2.boundingRect(contour)
        return w > h  # Consider horizontal if width is greater than height
