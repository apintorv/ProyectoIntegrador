...
class Control_Node(Node):
    ...
    def timer_callback(self):
        ...
        angle_error = self.thetha - np.arctan2(self.qd[1] - self.q0[1], self.qd[0] - self.q0[0])
        angle_error = (angle_error + np.pi) % (2 * np.pi) - np.pi
        if abs(angle_error) > 0.1:  # Si el error del angulo es mayor a un umbral en radianes
            self.twist.angular.z = -self.k * angle_error # control de orientación
            self.twist.linear.x = 0.0 # no avanzamos
        else:
            # Si el error del ángulo es pequeño, calculamos la velocidad lineal
        ...
        # if abs(angle_error) > 0.1:  # Si el ángulo no está alineado
        #     aux[1] = -0.2 * angle_error  # Control de orientación
        #     self.twist.linear.x = 0  # No avanzamos aún
        # else:
        #     if np.linalg.det(matrix_D) != 0:
        #         U = np.linalg.inv(matrix_D) @ aux
        #         self.twist.linear.x = U[0][0]  # Avanzamos hacia la posición
        #     self.twist.angular.z = 0  # Una vez alineados, no giramos
